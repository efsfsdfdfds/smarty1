  ...
  <form id='form' action="" method="post" enctype="multipart/form-data" style="width: 80%;height: 800px;margin: 80px auto 80px auto">
            <td class="active" style="width: 120px">
              <select class="form-control" style="width: 120px" name='class'>
                   {foreach $arr as $v}
                     <option value="{$v['id']}" {if {$v['id']}=={$arr1['classid']}}{'selected'}{else}{''}{/if}>{str_repeat('->',$v['num'])}{$v['title']}</option>
                  {/foreach}
              </select>
          </td>

        <input type="hidden" name="id" value="{$arr1['id']}"
        <h3>商品的添加</h3>
        <span>商品名称</span>
        <input type="text" class="form-control required" value="{$arr1['name']}"  name="user" style="width:50%" >
        ...
        {foreach $arr2 as $vv}
          <input type="text" value="{$vv['stylename']}" class='aa' name='styleup[]' style="width:20%"><input type="hidden" name="styleid[]" value="{$vv['id']}">&nbsp;&nbsp;属性&nbsp;&nbsp;<a href="delss.php?id={$vv['id']}&gid={$arr1['id']}" onclick="return del()">删除属性</a><br>
        {/foreach}
        <span>商品封面图</span>
        <input type="file" name="idcard[]" onchange='jjj(this)'>
        <div id="preview">
            <img id="imghead" width=100 height=100 border=0 src='../upload/{$arr1['pic']}'>
         </div>
         <span>商品封面图</span>
        <input type="file" name="idcard[]" onchange='jjjj(this)'>
        <div id="previews">
            <img id="imghead" width=100 height=100 border=0 src='../upload/{$arr1['pic2']}'>
         </div>
        <span>商品的价格</span>
        <input type="text" class="form-control required" value='{$arr1['price']}' name="price" style="width:20%">
        <span>商品的库存数量</span>
        <input type="text" class="form-control required" value="{$arr1['mun']}" name="mun" style="width:20%">
        <span>商品的产地</span>
        ...
        <div>
          ...
          <script type="text/javascript">
            addressInit('cmbProvince', 'cmbCity', 'cmbArea', '{$add[0]}', '{$add[1]}', '{$add[2]}');
